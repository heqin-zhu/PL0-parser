...
def parse_args():
    ...
    ap.add_argument('-i', '--instruction',
                    help="output instructions", action='store_true')
    ap.add_argument(
        '-s', '--stack', help="output data stack when executing each instruction", action='store_true')
    ap.add_argument(
        '-t', '--token', help="output tokens when parsing", action='store_true')
    ap.add_argument(
        '-v', '--varible', help="output varibles for every static environment", action='store_true')
    ap.add_argument(
        '-f', '--file', help="compile and run codes. \n Without this arg, enter interactive REPL", type=str)
    return ap.parse_args()


SHOWINS = SHOWSTACK = SHOWVAR = SHOWTOKEN = False
...
class closure:
    ...
    def __setitem__(self, key, val):
        if key in self.items:
            raise Exception('[Error]: {} has been defined'.format(key))
        if val.type == 'VAR':
            self.varNum += 1
        self.items[key] = val
...
class parser(object):
    ...
    def initSymbol(self, syms=None):
        if syms is None:
            syms = [symbol('E', 'CONST', e, 0), symbol('PI', 'CONST', pi, 0)]
    ...
    def errorArg(self, n1, n2):
        raise Exception(
            '[Error]: Expected {} args, but {} given'.format(n1, n2))

    def errorExpect(self, s):
        raise Exception('[Error]: Expected {}, got "{}"'.format(
            s, self.tokens[self.pointer].value))
    ...
    def parse(self, tokens=None):
        ...
        try:
            ...
            if SHOWINS:
                print('     ins   i   a')
                for i, ins in enumerate(self.codes):
                    print(str(i).ljust(4), ins)
    ...
    def isType(self, s):
        ...
        if s in self.reserved:
            return sym.value == s.lower()
        if s == 'NAME' and sym.value.upper() in self.reserved:
            return False
        return sym.type == s
    ...
    def wantType(self, s):
        if not self.isType(s):
            self.errorExpect(s)

    def backpatching(self, ip, addr, levelDiff=None):
        self.codes[ip].addr = addr
        if levelDiff is not None:
            self.codes[ip].levelDiff = levelDiff
...
class PL0(parser):
    def __init__(self, tokens=None, syms=None, codes=None, level=0):
        ...
        super().__init__()
        self.reserved = {'FUNC', 'PRINT', 'RETURN', 'BEGIN', 'END', 'IF', 'THEN', 'FOR', 'ELIF', 'ELSE',
                         'WHILE', 'DO', 'BREAK', 'CONTINUE', 'VAR', 'CONST', 'ODD', 'RANDOM', 'SWITCH', 'CASE', 'DEFAULT'}
        self.bodyFirst = self.reserved.copy()
        self.bodyFirst.remove('ODD')
        self.relationOPR = {'EQ': eq, 'NEQ': ne,
                            'GT': gt, 'LT': lt, 'GE': ge, 'LE': le}  # odd
        self.conditionOPR = {'AND': and_, 'OR': or_, 'NOT': not_}
        self.conditionOPR.update(self.relationOPR)
        self.arithmeticOPR = {'ADD': add, 'SUB': sub, 'MOD': mod, 'MUL': mul,
                              'POW': pow, 'DIV': lambda x, y: x/y, 'INTDIV': lambda x, y: round(x)//round(y)}
        self.bitOPR = {'LSHIFT': lambda x, y: round(x) << round(y), 'RSHIFT': lambda x, y: round(x) >> round(
            y), 'BITAND': lambda x, y: round(x) & round(y), 'BITOR': lambda x, y: round(x) | round(y), 'BITNOT': lambda x: ~round(x)}
        self.binaryOPR = dict()
        ...
        self.unaryOPR = {'NEG': neg, 'NOT': not_, 'BITNOT': lambda x: ~round(x), 'FAC': lambda x: reduce(mul, range(
            1, round(x)+1), 1), 'ODD': lambda x: round(x) % 2 == 1, 'RND': lambda x: randint(0, x), 'INT': round}  # abs
    ...
    def sentence(self, outerLoop=None):
        ...
        if self.isType('BEGIN'):
            ...
            for seg in segs:
                if seg in ['%d', '%f']:
                    ...
                    self.sentenceValue()
                    if seg == '%d':
                        self.genIns('OPR', 1, 'INT')  # type convert
                    n += 1
                else:
                    for i in seg:
                        self.genIns('LIT', 0, i)
        ...
        elif self.isAnyType(['SEMICOLON', 'END', 'ELSE']):
            pass  # allow blank sentence: namely   ; ;;
    ...
    def assignment(self):
        ...
        for var in varLst[1:]:
            ...
            if sym.type == 'CONST':
                raise Exception(
                    '[Error]: Const "{}" can\'t be reassigned'.format(sym.name))
    ...
    def interpret(self):
        ...
        while 1:
            ...
            if ins.name == 'INT':
                if ins.levelDiff == 0:
                    stk.top += ins.addr-3  # allocate space
                elif ins.levelDiff == 1:
                    stk.top -= ins.addr  # rewind stack top bakc n spaces
                ...
                self.errorIns(ins, pc-1)
            if SHOWSTACK:
                print(str(pc).ljust(5), ins, stk[:stk.top+1])
            if pc == 0:
                break
        ...
        try:
            while 1:
                ...
                if len(lines) == 1:
                    ...
                    print()
                tk = [i for i in genToken(''.join(lines))]
                if tk == []:
                    continue
                res = pl.parse(tk)
        except EOFError:
